...
describe("/users", () => {
  ...
  test("DELETE /users/:id -  Não deve excluir a conta do vendedor sem autenticação", async () => {
    const adminLoginResponse = await request(app).post("/login").send(mockedSellerLogin);
    const UserTobeDeleted = await request(app).get('/users/profile').set("Authorization", `Bearer ${adminLoginResponse.body.token}`)

    const response = await request(app).delete(`/users/profile${UserTobeDeleted.body.id}`)
  ...
  test("DELETE /users/profile -  Deve deletar o perfil do vendedor", async () => {
    ...
    const findUsers = await request(app).get('/users').set("Authorization", `Bearer ${loginResponse.body.token}`)
    expect(response.status).toBe(204)
    expect(findUsers.body).toBe({})
  ...
  test("PATCH /users/:id -  Deve atualizar os dados do vendedor", async () => {
